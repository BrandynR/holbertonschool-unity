...
public class Swipe : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        transform.position = ammoHolder.position;
    }
    ...
    { 
        if (shotAmmo)
        {
            if (transform.position.y < SelectPlane.chosenPlane.transform.position.y)
        ...
        if (holdTouch)
        {
            ...
            ammoOrigin = Camera.main.ScreenToWorldPoint( new Vector3(Screen.width/2, Screen.height/2, 1f)); //Camera.main.nearClipPlane + 1f));
