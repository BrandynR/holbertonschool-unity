...
public class PredictionManager : MonoBehaviour
{
    ...
    {
        GameObject plane = Instantiate(SelectPlane.chosenPlane.gameObject);
        plane.transform.position = SelectPlane.chosenPlane.transform.position;
        plane.transform.rotation = SelectPlane.chosenPlane.transform.rotation;
    ...
    public void Predict(GameObject subject, Vector3 currentPosition, Vector3 force, LineRenderer lineRenderer)
    {
        if (currentPhysicsScene.IsValid() && predictionPhysicsScene.IsValid())
        {
            if (dummy == null)
            {
                ...
                dummy.GetComponent<Swipe>().enabled = false;
            }
